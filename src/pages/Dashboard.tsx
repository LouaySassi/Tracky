...
export function Dashboard({
  ...
  const navigateMonth = (dir: 'prev' | 'next') => {
    ...
    if (!allMonthsData[newKey]) {
      const template = currentData || getEmptyMonthData(settings.defaultSalary)
      setAllMonthsData((prev) => ({
        ...prev,
        [newKey]: createMonthDataFromTemplate(template),
      }))
    }
  }
  ...
  const handleUpdateBillBudget = (id: string, budget: number) => {
    updateCurrentMonth((prev) => ({
      ...prev,
      monthlyBills: prev.monthlyBills.map((bill) =>
        bill.id === id
          ? {
              ...bill,
              budget,
  ...
  const handleRemoveBillFromAllMonths = (id: string) => {
    setAllMonthsData((prev) => {
      const updated: MonthlyDataStore = {}
      Object.entries(prev).forEach(([key, data]) => {
        updated[key] = {
          ...data,
          monthlyBills: data.monthlyBills.filter((b) => b.id !== id),
          transactions: data.transactions.filter((t) => t.itemId !== id),
        }
      })
      return updated
    })
  ...
  const handleAddBillPayment = (id: string, amount: number, note?: string) => {
    updateCurrentMonth((prev) => {
      ...
      const updatedBills = prev.monthlyBills.map((b) =>
        b.id === id
          ? {
              ...b,
      ...
      if (transaction.type === 'bill') {
        updatedData.monthlyBills = prev.monthlyBills.map((b) => {
          if (b.id === transaction.itemId) {
            ...
            if (bill.linkedGoalId) {
              updatedData.goals = prev.goals.map((g) =>
                g.id === bill.linkedGoalId
                  ? {
                      ...g,
                      currentAmount: Math.max(
                        0,
                        g.currentAmount - transaction.amount,
                      ),
                    }
                  : g,
              )
        ...
        updatedData.expenses = prev.expenses.filter(
          (e) => e.id !== transaction.itemId,
        )
      } else if (transaction.type === 'savings') {
        updatedData.totalSavings = Math.max(
          0,
          prev.totalSavings - transaction.amount,
        )
      } else if (transaction.type === 'extra-funds') {
        updatedData.extraFunds = Math.max(
          0,
          prev.extraFunds - transaction.amount,
        )
      }
      updatedData.transactions = prev.transactions.filter(
  ...
  const getAllSavingsTransactions = () => {
    return Object.values(allMonthsData)
  ...
  const calculateRemainingFunds = () => {
    ...
    const totalBillsSpent = currentData.monthlyBills.reduce(
      (sum, b) => sum + b.spent,
      0,
    )
    ...
    return (
      currentData.monthlySalary +
      currentData.extraFunds -
      totalBillsSpent -
      totalExpenses -
      manualSavingsThisMonth
    )
  }
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          <div className="mb-12">
            <RemainingFunds
              amount={calculateRemainingFunds()}
              salary={currentData.monthlySalary || 0}
              extraFunds={currentData.extraFunds}
              onUpdateSalary={() => setIsSalaryModalOpen(true)}
              onAddExtraFunds={() => setIsExtraFundsModalOpen(true)}
            />
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentData.monthlyBills.map((bill) => (
                <BillCard
                  key={bill.id}
                  bill={bill}
                  transactions={getTransactionsForItem(bill.id)}
                  onUpdateBudget={(budget) =>
                    handleUpdateBillBudget(bill.id, budget)
                  }
                  onAddPayment={(amount, note) =>
                    handleAddBillPayment(bill.id, amount, note)
                  }
                  onDeleteTransaction={handleDeleteTransaction}
                  onRemove={() => handleRemoveBill(bill.id)}
                  onRemoveFromAllMonths={() => handleRemoveBillFromAllMonths(bill.id)}
                />
              ))}
            </div>
          </div>
          ...
          <div className="mb-8">
            <SimpleSavingsCard
              totalSavings={currentData.totalSavings}
              allTransactions={getAllSavingsTransactions()}
          ...
          <div className="mb-8">
            <GoalsSection
              ...
              onUpdateMonthlyPayment={handleUpdateGoalMonthlyPayment}
            />
          </div>
      ...
      <SalaryInput
        ...
        onSave={handleUpdateDefaultSalary}
        currentSalary={settings.defaultSalary}
      />

      <Modal
        isOpen={isExtraFundsModalOpen}
        onClose={() => setIsExtraFundsModalOpen(false)}
        title="Extra Funds"
      >
        <div className="space-y-4">
          ...
          <Input
            type="number"
            value={extraFundsAmount}
            onChange={setExtraFundsAmount}
            placeholder="Amount"
            label="Amount"
            fullWidth
            autoFocus
          ...
          <Input
            type="text"
            value={extraFundsNote}
            onChange={setExtraFundsNote}
          ...
          {currentData.transactions.filter((t) => t.type === 'extra-funds')
            .length > 0 && (
              ...
              <div className="space-y-2">
                {currentData.transactions
                  ...
                  .map((transaction) => (
                    <div
                      key={transaction.id}
                      className="flex items-center justify-between p-3 bg-dark-bg rounded-lg hover:bg-dark-card-hover transition-colors group"
                    >
